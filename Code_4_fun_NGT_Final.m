function [] = Code_4_fun_NGT(X,y,z,XT,yT,zT,F,VecEps,NumT,Nsim,NumIter,NumFev,NumPoints,a_ini,b_ini,beta0_ini,invSigma_ini,NameNGT)
...
for i=1:length(VecEps)  
    ...
    if F==1
        Sol = Code_3_NGT(X,y,z,VecEps(i),NumT,NumIter,NumFev,NumPoints,a_ini{i},b_ini{i},beta0_ini{i},invSigma_ini{i});
    else
        Sol = Code_3_NGT(X,y,zT,VecEps(i),NumT,NumIter,NumFev,NumPoints,a_ini{i},b_ini{i},beta0_ini{i},invSigma_ini{i});
    end
    ...
    a_opt = Mat(1);               % Value of a at solution
    b_opt = Mat(2);               % Value of b at solution
    beta0_opt = Mat(3:3+p);           % Value of beta0 at solution
    invSigma_opt = zeros(p+1);         % Value of Sigma at solution
    k0=1;
    for l=1:p+1
        for j=l:p+1
            invSigma_opt(l,j)=Mat(p+3+k0);
            k0=k0+1;
        end
    end 
    ...
    if i>1 && MargLik<MargLik_Def{i-1}      
        ...
        invSigma_ini_Def{i}=invSigma_ini_Def{i-1};
        ...
        invSigma_ini_Def{i}=invSigma_opt;
