...
function [] = Code_4_fun_NGD_Final(X,y,z,XT,yT,zT,F,VecEps,NumT,Nsim,NumIter,NumFev,NumPoints,a_ini,b_ini,beta0_ini,invSigma_ini,NameNGD)
...
for i=1:length(VecEps)  
    ...
    if F==1
        Sol = Code_3_NGD(X,y,z,VecEps(i),NumT,NumIter,NumFev,NumPoints,a_ini,b_ini,beta0_ini,invSigma_ini);
    else
        Sol = Code_3_NGD(X,y,zT,VecEps(i),NumT,NumIter,NumFev,NumPoints,a_ini,b_ini,beta0_ini,invSigma_ini);
    end
    ...
    b_opt = Mat(1,2);
    beta0_opt = Mat(:,3);
    invSigma_opt_aux=Mat(:,4);
    invSigma_opt = diag(Mat(:,4));
    ...
    if i>1 && MargLik<MargLik_Def{i-1}      
        ...
        invSigma_ini=diag(invSigma_ini_Def{i-1});
